...
export default function FulizaCalculator() {
  ...
  function calculate() {
    ...
    if (value > 70000) {
      setError("Maximum Fuliza amount is KES 70,000.");
      return;
    }
    ...
    if (mode === "owe") {
      ...
      daysElapsed = Math.max(
        1,
        Math.floor((today - borrowed) / (1000 * 60 * 60 * 24)) + 1,
      );
      if (daysElapsed > 30) {
        setError(
          "You will not have access to your Fuliza limit if you have any unpaid balance after Day 30. Repay your full balance to restore your limit — or call Safaricom on 100.",
        );
        return;
      }
    }
    ...
    if (!calc) {
      setError("Amount outside supported range.");
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {mode === "plan"
            ? "AMOUNT TO FULIZA (KES)"
            : "AMOUNT YOU FULIZA'D (KES)"}
        </div>
          ...
          >
            ...
            <div style={{ padding: "16px" }}>
              {[
                {
                  label: "Amount",
                  value: `KES ${result.amount.toLocaleString()}`,
                },
                {
                  label: "Access fee (1%)",
                  value: `KES ${result.accessFee.toLocaleString()}`,
                },
                {
                  label: "Grace days",
                  value:
                    result.graceDays > 0
                      ? `${result.graceDays} days free`
                      : "None",
                  green: result.graceDays > 0,
                },
                {
                  label: `Daily fee × ${result.chargeableDays} days`,
                  value:
                    result.dailyTotal === 0
                      ? "Free"
                      : `KES ${result.dailyTotal.toLocaleString()}`,
                  green: result.dailyTotal === 0,
                },
                {
                  label: "Total cost",
                  value: `KES ${result.totalCost.toLocaleString()}`,
                  red: true,
                },
              ].map((row) => (
                <div
                  ...
                  style={{
                    ...
                    padding: "7px 0",
                    borderBottom: "0.5px solid var(--receipt-divider)",
                  }}
                >
                  <span
              ...
              >
                ...
                >
                  KES {result.totalRepay.toLocaleString()}
                </span>
              </div>
        ...
        <div style={{ marginTop: "12px" }}>
          <button
            className="crb-toggle"
            onClick={() => setShowCrb(!showCrb)}
            style={{
              width: "100%",
              padding: "12px 16px",
              borderRadius: "10px",
          ...
          >
            <span>
              ...
              ></i>
              What happens if you're listed on CRB?
            </span>
            <span>{showCrb ? "↑" : "↓"}</span>
          </button>
          ...
          {showCrb && (
            ...
            >
              {[
                {
                  emoji: "fa-circle-check",
                  title: "Positive listing",
                  color: "#1A7A4A",
                  desc: "You pay on time. This builds your credit score and helps you access better loans, higher limits and lower interest rates in future.",
                },
                {
                  emoji: "fa-circle-xmark",
                  title: "Negative listing",
                  color: "var(--warn-color)",
                  desc: "You defaulted. Lenders see this and will reject loan applications, offer worse terms or charge higher interest. Stays for up to 5 years.",
                },
                {
                  emoji: "fa-ban",
                  title: "What gets blocked",
                  color: "var(--label-color)",
                  desc: "Bank loans, mobile credit (M-Shwari, KCB M-Pesa, Tala, Branch), business tenders and in some cases employment opportunities.",
                },
                {
                  emoji: "fa-lock-open",
                  title: "How to clear it",
                  color: "var(--label-color)",
                  desc: "Pay off the outstanding Fuliza balance. Once settled the negative listing is removed. You can dispute incorrect listings with the CRB.",
                },
                {
                  emoji: "fa-file-lines",
                  title: "Your rights",
                  color: "var(--label-color)",
                  desc: "You're entitled to one free credit report per year. If a listing is wrong, dispute it in writing with the bureau within 30 days.",
                },
              ].map((item) => (
                <div
                  key={item.title}
              ...
              <div
                style={{
                  ...
                  marginTop: "8px",
                  borderTop: "0.5px solid var(--idle-border)",
                  paddingTop: "8px",
                }}
              >
        ...
        >
          Safaricom/NCBA official Fuliza tariffs · Updated per Key Facts
          Document
        </p>
      </div>
    </div>
